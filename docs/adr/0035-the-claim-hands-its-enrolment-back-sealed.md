...
**Superseded by
[ADR 0041](./0041-the-second-factor-is-offered-not-required.md), and then by
[ADR 0054](./0054-the-first-administrator-comes-from-the-environment.md).** The
claim no longer enrols anything, so it has nothing to hand back and no ticket to
seal; and then the claim itself is gone. The mechanism below was not wrong and
did not go away — it is the one described in
[ADR 0036](./0036-an-enrolment-carries-its-own-sealed-ticket.md), which is now
the only place an enrolment happens, behind a signed-in user rather than a
claimant. This document is kept because the reasoning about provenance is what
that one inherits.
