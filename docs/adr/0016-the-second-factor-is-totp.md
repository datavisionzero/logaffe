...
The second factor is **re-enrollable while signed in**, asking for the password
and either the current code or a backup code, and issuing a fresh set of backup
codes when it succeeds. Replacing a phone is an ordinary event, and a product
where it costs the installation would be one where operators avoid enrolling
properly. This is not in tension with
