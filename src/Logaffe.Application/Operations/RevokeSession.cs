...
/// <summary>
/// Ends one session from a user's own list.
/// </summary>
/// <remarks>
/// <para>
/// It removes the row, exactly as <see cref="SignOut"/> does and as revoking a
/// token does: the list is what a person acts on, and one they ended has to
...
/// </para>
/// <para>
/// <b>The row is found by reading that user's list</b>, which is also what
/// keeps somebody from ending a session of somebody else's by naming its id: a
/// row that is not in the caller's list is a row this does not find. There is no
/// lookup by id in the store and no reason to add one — an account holds a
/// handful of these, and a second way in would be a second thing to keep
/// honest.
/// </para>
/// <para>
/// <b>Ending the session making the request is allowed</b> and is a sign-out by
/// another name. Refusing it would be a rule somebody has to learn in order to
/// be protected from something that costs them one sign-in — but the adapter
